...
| `list_memory_records` | List this region's memory records directly, newest last, without going through search. Pass `principal` whenever you know the caller's identity — exactly as `memory_write` asks — and only the records that principal may see come back (`user`/`session`-scope records are visible only to their own writer; `org`-scope always is). Omit it and every record is returned, matching the `pheasant://…/memory` resource, which offers the same listing with no `principal` parameter since a resource URI carries no caller identity. |
