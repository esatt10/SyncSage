...
### The corpus behind those numbers

The tuning and effectiveness panels run over **[SciFact][scifact]** — one of
the BEIR tasks, and the small retrieval benchmark most open-source retrieval
stacks report on. 395 scientific abstracts (a quarter of them written as real
PDFs, so the extraction path is exercised by the same corpus the numbers come
from), 60 claims, and **66 expert relevance judgements**.

That last number is the point. The known-positives are annotations by domain
experts who read the abstract and marked the sentences supporting or
contradicting the claim — not positives invented by the seeding script. A demo
corpus whose judgements were written by its own fixture measures the fixture.

It also means the numbers are comparable to something outside this repository:
the text arm's 0.77 reciprocal rank is in the neighbourhood you would expect
from BM25 on SciFact. And the ablation shows something real rather than
flattering — **hybrid scores 0.68, below the text arm's 0.77**, because the
graph arm contributes almost nothing on scientific prose and dilutes the merge.
That is exactly the finding the mechanism table exists to surface.

The corpus is fetched at screenshot time and never vendored; the offline test
suite does not touch it and stays network-free.
...
evidence". The tuning diagnosis and its sweeps are likewise a real batch —
including, often, a decision to change nothing.
