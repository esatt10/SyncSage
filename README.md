...
For PR releases, comment with the release increment instead of manually editing version files. The publish workflow applies the selected increment to `main` before building the release image. CI runs `python scripts/sync_version.py --check`, and the release-version workflow checks existing GHCR image tags.
