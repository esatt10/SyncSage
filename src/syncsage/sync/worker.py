...
swung by 4x. Cooperative yielding (:mod:`syncsage.sync.pacing`) softened that;
it could not remove it, because the work and the serving still shared one
interpreter.

So the server does not index. It runs ``python -m syncsage sync`` as a child
...
lease (:class:`~syncsage.sync.locks.EngineLease`) the in-process path took, so
two writers still cannot overlap. ``SyncEngine.sync_source`` remains the
in-process path, used by the CLI, the tests, and anyone embedding SyncSage.
"""
...
def run_sync(
    ...
    *,
    ...
    The report mirrors :class:`~syncsage.sync.engine.SyncResult` fields when
...
class WorkerBackedEngine:
    ...
    *index* are replaced, so the watcher, the scheduler and the startup pass
    ...
    def _can_delegate(self) -> bool:
        ...
        embedded use, `syncsage up`, and every test that calls
    ...
    def _adopt(self, report: dict[str, Any]) -> list[Any]:
        ...
        from syncsage.sync.engine import SyncResult
