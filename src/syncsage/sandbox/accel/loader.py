...
2. **A one-shot subprocess (`sync/worker.py` spawns a fresh
   `python -m syncsage sync` per sync — there is no "second call" within
...
from syncsage.sandbox.wasm_runtime import WasmRuntimeUnavailable
...
def _cache_dir() -> Path:
    return Path(tempfile.gettempdir()) / "syncsage-wasm-cache"
...
def _engine_and_module() -> tuple[wasmtime.Engine, wasmtime.Module]:
    ...
    if wasmtime is None:
        raise WasmRuntimeUnavailable(
            ...
            "Install with: pip install 'syncsage[wasm]'"
        )
    with _lock:
        if _module is None:
