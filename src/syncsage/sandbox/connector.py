...
(``tests/fixtures/syncsage-connector-example``'s ``StaticDirConnector``) to
run its per-item content transform inside a :class:`~syncsage.sandbox.
...
:func:`syncsage.security.path_policy.resolve_under`, reused rather than
...
from syncsage.sandbox.wasm_runtime import HostCapabilities, WasmSandbox
