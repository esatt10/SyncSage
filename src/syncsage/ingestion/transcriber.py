...
the audio twin of :mod:`syncsage.ingestion.captioner` (image, session A) —
...
Idempotency note
----------------
Transcription happens inside :func:`syncsage.ingestion.pipeline.parse_*`, but
...
from syncsage.ingestion._modal import sidecar_text, stub_fingerprint
...
class OpenAISpecTranscriber:
    ...
    def transcribe(self, content: bytes, relative_path: str, sidecar: bytes | None = None) -> str:
        ...
        headers = {"Content-Type": content_type, "User-Agent": "SyncSage/0.1"}
    ...
    def _multipart(self, content: bytes, relative_path: str) -> tuple[bytes, str]:
        boundary = f"----SyncSage{uuid.uuid4().hex}"
...
def source_includes_audio(source: Any) -> bool:
    ...
    from syncsage.ingestion.content_types import AUDIO_EXTENSIONS
...
def transcriber_from_config(config: Any) -> Transcriber | None:
    ...
    if not config_has_audio_source(config):
        return None
    try:
