...
proxy. Reproduce the recorded numbers with::
    ...
    python -m syncsage.memory.benchmark
"""
...
def run_memory_recall_benchmark(
    ...
    """Run the four categories against a live ``SyncSageTools`` facade.
    ...
    kb = knowledge_base or tools.config.syncsage.name
...
def main() -> None:  # pragma: no cover - manual reproduction entry point
    ...
    with tempfile.TemporaryDirectory(prefix="syncsage-membench-") as tmp:
        root = Path(tmp)
        (root / "memory").mkdir()
        config_path = root / "syncsage.yaml"
