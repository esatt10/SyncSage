...
Third-party packages add a source type to pheasant without forking it by
declaring an entry point in the ``pheasant.connectors`` group::
...
``pheasant.yaml``. Embedders and tests can also register classes
...
``tests/fixtures/pheasant-connector-example/``; the public quality bar is
...
ENTRY_POINT_GROUP = "pheasant.connectors"
...
def register_connector_class(type_name: str, connector_class: type) -> None:
    ...
    if not _is_connector_class(connector_class):
        raise TypeError(
            ...
            "connector plugins must subclass pheasant.sync.connectors.SourceConnector"
        )
