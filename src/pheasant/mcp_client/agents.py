...
- ``local``: the pip-installed ``pheasant`` binary speaks MCP over stdio —
  the zero-docker path that pairs with ``pheasant up``.
...
from pheasant.mcp_client.vscode import docker_exec_stdio_config, docker_run_stdio_config
...
def local_stdio_server(config_path: str = "pheasant.yaml") -> dict[str, Any]:
    """The inner server entry for a locally installed pheasant."""
    return {
        "command": "pheasant",
